...
export default function ProspectDetailPage() {
  ...
  const visibleActivities = showAllActivity ? filteredActivities : unreadActivities;
  const readActivityCount = filteredActivities.length - unreadActivities.length;
  const visibleSharedNotes = showAllSharedNotes ? comments : unreadSharedNotes;
        ...
        <div className="prospectDetailLayout">
          <section className="prospectDetailMain">
            ...
            >
              ...
              <section className="prospectOpsPanel" hidden={detailView !== "activity"}>
                ...
                <div className={visibleActivities.length > ACTIVITY_PREVIEW_LIMIT ? "prospectTimeline prospectTimelineScrollable" : "prospectTimeline"}>
                ...
                {readActivityCount > 0 ? <button className="button buttonSecondary prospectActivityMore" onClick={() => setShowAllActivity((current) => !current)}>{showAllActivity ? "Hide past activity" : `View past activity (${readActivityCount})`}</button> : null}
              </section>
            </div>
            ...
            {prospect?.workspace_id ? <section className="prospectOpsPanel prospectCommentsPanel" hidden={detailView !== "activity"}>
              ...
              {comments.length ? <p className="small prospectListLimitNote">{showAllSharedNotes ? `Showing all ${comments.length} shared notes.` : newSharedNoteCount ? `Showing ${newSharedNoteCount} new shared ${newSharedNoteCount === 1 ? "note" : "notes"}.` : "No new shared notes."}</p> : null}
              <div className={visibleSharedNotes.length > SHARED_NOTES_PREVIEW_LIMIT ? "prospectTimeline prospectTimelineScrollable" : "prospectTimeline"}>
              ...
              {readSharedNoteCount > 0 ? <button className="button buttonSecondary prospectActivityMore" onClick={() => setShowAllSharedNotes((current) => !current)}>{showAllSharedNotes ? "Hide past notes" : `View past notes (${readSharedNoteCount})`}</button> : null}
            </section> : null}
          ...
          <aside className="prospectActionPanel">
            ...
            <div style={{ display: "grid", gap: 10, marginTop: 18 }}>
              ...
              {id ? <Link className="button buttonSecondary" href={`/client-folders/${id}`}>Open saved folder</Link> : null}
